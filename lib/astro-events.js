...
import sunJupiterData from '@/src/data/sun-jupiter-tracking.json';
...
export function getMarchEquinoxEvents() {
  return (marchEquinox.entries || []).map((entry, index) => {
    const [day, month] = entry.date.split(' ');
    const monthNumber = new Date(`${month} 1, ${entry.year} UTC`).getUTCMonth() + 1;
    const eventDate = `${entry.year}-${String(monthNumber).padStart(2, '0')}-${day.padStart(2, '0')}`;
    return {
      id: `march-equinox-${eventDate}-${index}`,
      type: 'EQUINOX',
      title: 'March Equinox',
      shortTitle: 'March Equinox',
      eventDate,
      displayDate: eventDate,
      description: `Indian time: ${entry.indianTime}`,
      startTime: entry.softwareTime,
      sourcePage: 'March Equinox',
      route: '/degree-calculator',
      icon: 'bi-sunrise',
      priority: 'HIGH',
    };
...
export function getSunJupiterEvents() {
  return (sunJupiterData.events || []).map((event, index) => ({
    id: `sun-jupiter-${event.date}-${index}`,
    type: 'SUN_JUPITER',
    title: 'Sun-Jupiter Tracking',
    shortTitle: `${event.angle}° ${event.aspect}`,
    eventDate: event.date,
    displayDate: event.date,
    description: `${event.aspect} · ${event.timeIST} IST`,
    startTime: event.timeIST,
    sourcePage: 'Sun-Jupiter Tracking',
    route: '/sun-jupiter-tracking',
    icon: 'bi-sun',
    priority: 'HIGH',
    degree: event.angle,
  }));
...
export function getAstroEvents() {
  const events = [...getPurnimaEvents(), ...getAmavasyaEvents(), ...getMarchEquinoxEvents(), ...getDegreeEvents(), ...getBhadraEvents(), ...getPanchakEvents(), ...getMangalEvents(), ...getShukraEvents(), ...getSunJupiterEvents(), ...getWeeklyMarketEvents()];
  return events.map((event) => ({
    ...event,
    category: event.category || (['PURNIMA', 'AMAVASYA', 'EQUINOX'].includes(event.type) ? 'Lunar & Calendar' : event.type === 'BHADRA' ? 'Bhadra' : event.type === 'DEGREE' && event.sourcePage === 'Degree Calculator' ? 'Market Analysis' : 'Planetary Analysis'),
  }));
}
